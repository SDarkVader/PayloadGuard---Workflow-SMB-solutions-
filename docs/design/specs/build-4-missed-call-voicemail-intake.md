...
**Status: fully working in production (2026-08-28).** A real test call surfaced a genuine architecture gap — Vapi's "Structured Outputs" feature (what the assistant actually uses) is computed asynchronously and never included in the `end-of-call-report` webhook payload; it has to be fetched via a follow-up call to Vapi's API. Fixed, and the fix confirmed three times over: against the real call's data directly via the API, replayed through the full route locally, and replayed through the full route against the live production deployment — DB row, Slack card, and delivered SMS all correct each time. Emergency-branch routing is a separate, later piece — not this route. See §11.
...
8. ~~First genuinely fresh real call~~ — done. DB write and Slack card confirmed correct; SMS failed (Twilio 400). Root cause: the caller gave a made-up test number, which was also missing a digit for a valid UK mobile — not a code bug on that specific case. But it surfaced a real gap while investigating: `sendConfirmationSms()` sent whatever format the phone number arrived in (this one in UK local format, leading `0`) straight to Twilio's `To` field, which requires E.164 (`+44...`). Fixed — `lib/sms.ts` now normalizes via `toE164()` before sending. A genuinely short/invalid number will still correctly fail at Twilio (this fix reformats, it doesn't fabricate digits), but a normal `07...` number will now work where it would have failed before.
