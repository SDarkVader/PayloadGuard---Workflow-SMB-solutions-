...
**Status: fully working in production, confirmed against real calls (2026-08-28).** An early real test call surfaced a genuine architecture gap — Vapi's "Structured Outputs" feature (what the assistant actually uses) is computed asynchronously and never included in the `end-of-call-report` webhook payload; it has to be fetched via a follow-up call to Vapi's API. Fixed. A later real call also surfaced a phone-format gap in the SMS path (UK local format sent to Twilio instead of E.164) — also fixed. A subsequent real call with a genuine phone number confirmed all three channels — DB, Slack, SMS — working correctly end-to-end, no replay or synthetic data involved. Emergency-branch routing is a separate, later piece — not this route. See §11.
...
6. ~~Add `VAPI_API_KEY` to Vercel's production environment~~ — done, redeployed, and confirmed: replayed the real call's ID against the live production endpoint and got a correct DB row, Slack card, and a delivered SMS, all matching the local test exactly. Every piece of this build is now individually proven working in production.
...
8. ~~First genuinely fresh real call~~ — done, across three real calls in total. First: DB and Slack correct, SMS failed (Twilio 400) — root cause was a phone-format gap (`sendConfirmationSms()` sent UK local format straight to Twilio instead of E.164), fixed via `lib/sms.ts`'s `toE164()`. Second: confirmed the fix against Twilio's own message log (number now correctly normalized) — came back `undelivered` only because that test used a fake number, not a real handset. Third, with a genuinely real phone number: DB, Slack, and SMS all confirmed working, delivered text received. **Build 4's general-enquiries branch is now fully proven end-to-end against real calls, not just synthetic or replayed data.**
