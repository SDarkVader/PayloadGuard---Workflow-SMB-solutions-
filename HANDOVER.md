...
3. **Build 4's general-enquiries voice intake is fully working in production.** A real test call surfaced that Vapi's "Structured Outputs" (what the assistant actually uses) are computed asynchronously and never appear in the `end-of-call-report` webhook — they have to be fetched afterward via Vapi's Call API. Fixed in `lib/vapi.ts`/`route.ts`, `VAPI_API_KEY` added to Vercel, and the fix confirmed by replaying the real call's ID against the live production endpoint: correct DB row, correct Slack card, and a delivered SMS, all matching what the local test already showed.
   - First genuinely fresh real call: DB and Slack both correct. SMS failed — Twilio rejected the phone number because `sendConfirmationSms()` sent it in whatever format it arrived (UK local `0...`) rather than the E.164 (`+44...`) Twilio requires. Fixed (`lib/sms.ts`'s new `toE164()`). A second real call (also fake test details) confirmed the fix directly against Twilio's own message log — number correctly normalized this time, Twilio accepted it, came back `undelivered` only because the test number wasn't a real handset (Twilio error 30003). SMS to a genuine number was already proven separately earlier in this build. All three channels (DB, Slack, SMS) are now fully confirmed.
