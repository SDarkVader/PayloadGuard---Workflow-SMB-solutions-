...
**Last updated:** 2026-08-28 (Build 1's core loop complete and verified; Build 4's general-enquiries voice intake done — three real test calls surfaced and led to fixing two genuine gaps (async Structured Outputs, SMS phone-number formatting), with the final call confirming DB, Slack, and SMS all working correctly against a real phone number)
...
3. **Build 4's general-enquiries voice intake is fully working in production.** A real test call surfaced that Vapi's "Structured Outputs" (what the assistant actually uses) are computed asynchronously and never appear in the `end-of-call-report` webhook — they have to be fetched afterward via Vapi's Call API. Fixed in `lib/vapi.ts`/`route.ts`, `VAPI_API_KEY` added to Vercel, and the fix confirmed by replaying the real call's ID against the live production endpoint: correct DB row, correct Slack card, and a delivered SMS, all matching what the local test already showed.
   - Three real calls in total: first found and led to fixing the Structured Outputs gap; second found and led to fixing an SMS phone-format gap (UK local sent to Twilio instead of E.164 — `lib/sms.ts`'s `toE164()`); third, with a genuine phone number, confirmed all three channels (DB, Slack, SMS) working correctly with no replay or synthetic data involved. **Build 4's general-enquiries branch is done and proven.**
